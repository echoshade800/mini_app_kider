...
const GameBoard = ({ 
  ...
  const getTileAtPosition = (x, y) => {
    ...
    const { boardLeft, boardTop, boardPadding, tileSize, tileGap, gridRows, gridCols } = fixedLayout;
    
    const relativeX = x - boardLeft - boardPadding;
    const relativeY = y - boardTop - boardPadding;
  ...
  const panResponder = PanResponder.create({
    onStartShouldSetPanResponder: (evt) => {
      ...
      const tile = getTileAtPosition(pageX, pageY);
      return tile !== null;
    },
    ...
    onPanResponderGrant: (evt) => {
      ...
      selectionOpacity.setValue(0.3);
      
      const newHoveredSet = new Set([tile.index]);
      setHoveredTiles(newHoveredSet);
      scaleTile(tile.index, 1.05);
    ...
    onPanResponderMove: (evt) => {
      ...
      if (endRow === tile.row && endCol === tile.col) return;
      
      hoveredTiles.forEach(index => {
        if (!getSelectedTilesForSelection({ ...selection, endRow: tile.row, endCol: tile.col }).some(t => t.index === index)) {
          scaleTile(index, 1);
        }
      });

      // Update hovered tiles with scaling effect
      const newSelection = { ...selection, endRow: tile.row, endCol: tile.col };
