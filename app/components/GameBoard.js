...
function calculateEffectiveAreaLayout() {
  const GameBoard = ({ 
    ...
    // Get tile rotation for sticky note effect
    const getTileRotation = (row, col) => {
      const seed = row * 13 + col * 7;
      return ((seed % 7) - 3) * 0.8; // -2.4 to 2.4 degrees
    };

    // Get fixed board layout
    ...
    // Get selected tiles based on current selection
    ...
    const panResponder = PanResponder.create({
      ...
      onStartShouldSetPanResponderCapture: (evt) => {
        ...
        const buttonAreaBottom = screenHeight - 40;
